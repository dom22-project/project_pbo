#!/usr/bin/env python3
"""
Simple restore without Flask initialization
"""
...
def simple_restore():
    """Direct MySQL restore bypassing Flask initialization"""
    
    csv_file = 'data/operation_tables_export_20260210_161040.csv'
    
    print("[RESTORE] === Simple Direct MySQL Restore ===")
    ...
    try:
        ...
        # Truncate table with foreign key check disabled
        print("[RESTORE] Step 2: Truncating table...")
        cursor.execute("SET FOREIGN_KEY_CHECKS=0")
        cursor.execute("TRUNCATE TABLE operation_tables")
        cursor.execute("SET FOREIGN_KEY_CHECKS=1")
        conn.commit()
        print("[RESTORE] ✓ Table truncated\n")
        ...
        for idx, (kode, op) in enumerate(operations.items(), 1):
            try:
                # Parse prices
                try:
                    dokter = float(str(op.get('biaya_dokter', 0) or 0).replace(',', '.').replace('E', 'e'))
                except (ValueError, TypeError):
                    dokter = 0
                
                try:
                    rs = float(str(op.get('biaya_rs', 0) or 0).replace(',', '.').replace('E', 'e'))
                except (ValueError, TypeError):
                    rs = 0
                ...
                skipped += 1
                if skipped <= 5:
                    print(f"  ERROR at row {idx}: {str(e)[:60]}")
                continue
        ...
        cursor.execute("SELECT kode, nama_tindakan, kelas, total_biaya FROM operation_tables LIMIT 3")
        print(f"\n  Sample operations:")
...
if __name__ == '__main__':
    print("[RESTORE] Direct MySQL Operation Tables Restore\n")
